...
struct Module {
    ...
    Module(const std::string& path_, uintptr_t base_, size_t size_, std::vector<Symbol> symbols_ = {},
           bool loaded_ = false)
        : path(path_), base(base_), size(size_), symbols(std::move(symbols_)), symbols_loaded(loaded_) {}
...
class Stacktrace {
  ...
  public:
    static Stacktrace capture(size_t max_frames = kMaxFrames) {
        ...
        const int captured = ::backtrace(st.frames_.data(), static_cast<int>(max_frames));
        st.size_ = captured > 0 ? static_cast<size_t>(captured) : 0u;
        return st;
    }
