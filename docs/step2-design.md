...
### 7.10 探测第三方 UMD 必须做进程隔离（2026-08-31）

把 7.9 的做法拿到板上跑，第二个候选节点直接把整个工具段错误带走了：

```
opened GBM device on /dev/dri/renderD129 (backend 'drm')
EGL 1.4 ready ... GL renderer 'softpipe'
dumb buffer ready: 256x256 XR24 ...
Segmentation fault
```

崩在 `eglCreateImageKHR` 附近，也就是第三方 `.so` 内部。后果是
**renderD130（唯一真正能用的那个节点）根本没被测到** —— 一个与本项目无关的
视频编解码节点的用户态驱动，决定了整台机器的探测结论。

#### 这件事进程内防不住

`probe_gl_nodes()` 做的事情，本质上是**把一堆来路不明的用户态驱动依次
加载进本进程**。其中一定会有从来没被这样用过的：别的 IP 的节点、
编解码节点、软件后备。它们崩在自己的 `.so` 里，没有任何返回值可以检查，
`expected` 也接不住。

所以每个候选 `fork()` 一个子进程去试，结果经管道回传定长 POD。
子进程被信号杀掉，就记一条 `crashed`，继续测下一个。

**这条记录本身就是结论**，不是工具故障：一个会崩的 UMD 就是不能用的 UMD。
`rank()` 里 `crashed` 直接返回 0。

#### 两个容易做错的细节

- **子进程自己 open KMS 节点，不用父进程传进来的 fd。** 共用 fd 的话，
  子进程崩在建完 fb 之后，那些 GEM 对象和 fb 会留在**父进程的 fd** 上，
  变成没人认领的内核对象。各开各的，崩溃随 fd 一起清干净。
- **`_exit` 而不是 `exit`。** 后者会 flush 从父进程继承来的 stdio 缓冲，
  同一行日志会打两遍。

`--no-isolate` 保留了进程内路径，唯一用途是拿 gdb 直接看崩在哪。

#### 更一般的一条

这一步之前，"运行时探测优于编译期假设"在本项目里的含义是
**"别猜，去问"**。这次补上了后半句：

> **去问的时候要假设被问的一方可能会崩。**

凡是要加载外部代码来回答的问题（UMD、固件、vendor 库），
探测器都得能在被问方崩掉之后继续工作，并把"它崩了"作为一个正常答案记下来。

### 7.11 排序判据里混进了一条恒真命题（2026-08-31）

带隔离的探测第一次跑全之后，选中的是 **card2 —— 显示节点自己**，
而不是 renderD130。分差来自 `scanout_accepted_by_kms` 那 2 分。

而对 card2 来说，这一项是**恒真的**：候选就是显示设备本身，
"显示设备能不能导入它自己分配的 buffer"当然是能。这不是一条能力，
是一条同义反复。

后果：一条空结论让显示节点压过了真正的渲染节点。**排名没错，判据错了。**

修法：`GlNode::same_device_as_kms` 标出这种情况，`rank()` 里这一项只在
跨设备成立时计分，表格里那一列打 `-` 而不是 `yes`，闸门报 SKIP 并写明
"这说明不了任何跨设备能力，用 `-r` 指定别的节点才能测"。

值得记下来的是这个错误的形状：**一个判据在某些输入下退化成恒真，
而它退化的时候恰好也是它最不该起作用的时候。** 探测器里的每一条判据
都该问一遍"有没有哪种输入让它白送分"。

顺带说明：选 card2 其实也能工作（显示节点上确实起得来硬件 GL，见
`docs/env.md` 关键点四），但那是这块板子上 Mesa 构建的巧合。
默认应该选真正的渲染节点，让分配设备与渲染设备分离的那条路被常态化地走到。

### 7.12 in_flight 有一个恒定的偏差（2026-08-31）
...
实际只有 1 次。多出来的那次是 **modeset**：它让第一块 buffer 开始被扫描
（所以确实要轮转到下一块），但它**没带 `PAGE_FLIP_EVENT`**，内核永远不会
为它投递完成事件。而 `mark_submitted()` 把"轮转"和"等一个事件回来"
这两件事绑在了一起。

所以 `in_flight` 从第一帧起就永久性地多 1。

改法：`mark_submitted(bool expects_event = true)`，modeset 那一次传 false。
`Swapchain` 和 `DumbBufferChain` 一起改 —— 两者语义刻意保持一致，
只改一个会埋下更难查的不一致。

**一个恒定的偏差比没有计数更糟**，因为它看起来像真的：
"退出时还有 2 次在飞"是一句完全合理的话，不去数一遍不会有人怀疑它。
这类计数器的正确性只能靠"手工算一遍应该是多少"来验证，
而这正是 Step 1 就把 ioctl 记账做成一等公民的理由。

### 7.13 非线性 modifier 被拒时不能降级（2026-08-31）

`Framebuffer::add_with_fallback()` 原来的行为是：`AddFB2WithModifiers`
返回 EINVAL 就丢掉 modifier 重试。这条路径在 ScanoutDevice 模式下
永远走不到（那边 modifier 恒为 INVALID），所以一直没暴露 ——
**它专门等着 RenderDevice 方向被打开的那一天。**

丢掉 modifier 意味着让驱动自己推断排布：

- 内存实际是线性的 → 驱动推断出线性 → 结果正确
- 内存实际是 tiling / 压缩的 → 驱动按线性去读 →
  **屏幕上是垃圾，而 addfb2 返回成功**

后者是这个项目最不能容忍的那类错误：**静默的错误结果**。
它比一个响亮的失败糟糕得多，因为没有任何东西会提示你去看这里。

改法：只有 `modifier == LINEAR` 允许降级，其它一律失败，
错误信息说明为什么不降级以及该怎么办。

**一般教训**：一条"降级"路径是否安全，取决于降级前后的语义差。
`LINEAR → 无 modifier` 的语义差接近零；
`某个 tiling → 无 modifier` 的语义差是"完全不同的内存解释"。
**把它们写成同一段代码是一个分类错误。**

在写降级路径时应该逐个问：**降级之后结果还正确吗？**
不正确就不该降级，该失败。

### 7.14 分配层的 modifier 协商循环（2026-08-31）

配合 7.13，`RenderDeviceSource::allocate()` 变成一个循环：
被 addfb2 拒绝的 modifier 从候选里去掉，重新分配，
直到收敛或候选用完（退到不带 modifier 的分配并 WARN）。

这是**能力协商的最小形式**：不预设哪个 modifier 好，
让两端各自否决，收敛到一个双方都接受的。
排序（"哪个更优"）仍然不在这一层，那是 Step 4 的 tranche 策略。

**副产品比功能本身更有价值**：被拒绝的 modifier 名单，
就是"IN_FORMATS 里报了但实际不能用于扫描输出"的清单。
Step 1 的勘察里已经发现 vsdrm 有两个私有 modifier 属于这一类，
当时是人工试出来的；现在这份数据会自己产生并打进日志。
